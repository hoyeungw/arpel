...
function isEmittingKeypress(stream) {
  const rtn = !!stream._keypressDecoder
  if (!rtn) {
 ...
 *
 * @param {NodeJS.WritableStream} stream writable stream instance
 * @api public
 */
export function enableMouse(stream) { stream.write('[?1000h') }
 ...
 * @api public
 */
export function disableMouse(stream) { stream.write('[?1000l') }

listenerCount = EventEmitter.listenerCount ?? ( (emitter, event) => emitter.listeners(event).length )
...
function emitKey(stream, s) {
  ...
  if (s === '\r') { key.name = 'return' } /* carriage return*/
  else if (s === '\n') {
    ...
    Array.prototype.forEach.call(s, c => emitKey(stream, c))
    return
  }
